...
/**
 * Texture sample generated by DirectShowMedia player.
 */
...
public:
	...
	/** Virtual destructor. */
	virtual ~FDirectShowMediaTextureSample() { }
...
public:
	...
	{
		if ((InBuffer == nullptr) || (InSampleFormat == EMediaTextureSampleFormat::Undefined) || (InSize <= 0) || (InStride <= 0) || InDim.X <= 0 || InDim.Y <= 0)
		{
			return false;
		}
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("invalid : %d * %d > %d"), InStride, InDim.Y, InSize)
			return false;
		}
...
/** Implements a pool for DirectShow texture samples. */
class FDirectShowMediaTextureSamplePool : public TMediaObjectPool<FDirectShowMediaTextureSample> { };
